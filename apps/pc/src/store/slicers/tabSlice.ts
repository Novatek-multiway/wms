...
import type { RootState, AppThunk } from "@/store/tabStore"; // 类型导入,并不造成 循环导入
...
export const demoAsync = createAsyncThunk("demo/fetch-data", async (data: any) => {
	const response = await new Promise<{ data: any }>(resolve => setTimeout(() => resolve({ data }), 200));
