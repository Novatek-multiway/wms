...
class ConfigStore {
	constructor() {
		...
		document.documentElement.classList.add(this.locale || '')
	}
	...
	multyTab = Boolean(localStorage.getItem("multyTab") === 'true');
	fullScreen = false;
	isAdmin = false;
	userName = 'admin'
	userPermissionData = [];
	webSocketIns: WebSocket | undefined = undefined;
	...
	// 动态用户菜单数据
	setUserPermissionData = (data: any) => {
		this.userPermissionData = data.permission;
		this.isAdmin = data.isAdmin;
		this.userName = data.realName
	};

	setWebSocketIns = (ws: WebSocket) => {
		this.webSocketIns = ws
	}
}
