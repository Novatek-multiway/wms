...
export function searchMenuValue(data: ISearchMenuProps): ISideMenu[] {
	...
	for (let i = 0; i < menus.length; i++) {
		...
		} else if (menus[i]?.label?.includes(value) && hasPermission(menus[i], permissions)) {
			currentPath.push({
				label: menus[i].label,
				path: splitPath(menus[i].key)
			});
 ...
 * @param permissions - 权限列表
 */
export function filterMenus(menus: ISideMenu[], permissions: string[]): ISideMenu[] {
	const result: ISideMenu[] = [];

	for (let i = 0; i < menus.length; i++) {
		// 处理子数组
		if (hasChildren(menus[i])) {
			const result = filterMenus(menus[i].children as ISideMenu[], permissions);
		...
		// 有权限或有子数据累加
		if (hasPermission(menus[i], permissions) || hasChildren(menus[i])) result.push(menus[i]);
	}
 ...
 */
export function getFirstMenu(menus: ISideMenu[], permissions: string[], result = ""): string {
	// 有结构时直接返回
	...
	for (let i = 0; i < menus.length; i++) {
		...
		if (hasChildren(menus[i]) && !result) {
			const childResult = getFirstMenu(menus[i].children as ISideMenu[], permissions, result);
